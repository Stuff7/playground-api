...
async fn it_fails_to_move_folder_inside_itself() {
  let (file_sys, _) = get_database().await;
  let ids = create_nested_folders(&file_sys).await;
  ...
  cleanup_files_collection(&file_sys).await;
}
...
async fn get_database() -> (FileSystem, Database) {
  ...
  (FileSystem(database.clone()), database)
}
...
async fn create_nested_folders(file_system: &FileSystem) -> Vec<String> {
  let mut ids = Vec::new();
  for i in 0..3 {
    let (new_file, _) = file_system
      .create_one(
        &File::new_folder(
          USER_ID1.into(),
          f!("Folder {i}"),
